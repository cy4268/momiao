...
export const daily = { user_id: '1', business_date: '2026-09-06', timezone: 'Asia/Shanghai', next_reset_at: '2026-09-06T16:00:00Z', amount: '500', amount_units: '250000000', asset: 'RESERVE_API_CREDIT', policy_version: '1', claimed: false, transaction_id: null, maintenance_active: false };
...
export function fixtureClient(respond?: (path: string, init?: RequestInit) => Response | Promise<Response> | undefined) {
    const fetcher = vi.fn(async (path: string, init?: RequestInit) => {
        ...
        if (path === '/platform/v1/rewards/hourly') return ok({ ...daily, reward_hour:'2026-09-06T00:00:00Z', next_reset_at:'2026-09-06T01:00:00Z', amount:'100', amount_units:'50000000', claims_today:0, daily_limit:24, accumulation:false });
        if (path === '/platform/v1/rewards/relief') return ok({ user_id:'1', amount:'300', amount_units:'150000000', asset:'RESERVE_API_CREDIT', policy_version:'1', threshold:'10', threshold_units:'5000000', current_total_assets:'500', current_total_assets_units:'250000000', assets_observed_at:'2026-09-06T00:00:00Z', eligible:false, cooldown_seconds:'14400', next_eligible_at:null, last_transaction_id:null, accumulation:false });
        if (path === '/api/v1/maintenance/notices') return ok({ items:[] });
