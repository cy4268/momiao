...
export interface BlackjackGameProps extends DirectExtraControls {
    snapshot: BlackjackProjectionDTO | null; autoResolving?: boolean;
...
function PlayingCard({ code }: { code: number }) {
    const suit = Math.floor(code / 13); const rank = ranks[code % 13];
    return <span className={`extra-playing-card${suit === 1 || suit === 2 ? ' is-red' : ''}`} data-testid="playing-card" role="img" aria-label={`${suitNames[suit]} ${rank}`}><span>{rank}<small>{suits[suit]}</small></span><b aria-hidden="true">{suits[suit]}</b></span>;
}
...
export function BlackjackGame(props: BlackjackGameProps) {
    ...
    return <section className="extra-game extra-blackjack" aria-labelledby={title} aria-busy={props.busy || command.pending}>
        ...
        <div className="extra-layout">
            <div className="extra-royal-table">
                <div className="extra-dealer" role="group" aria-label="庄家公开手牌"><p className="extra-stage-label">DEALER · 系统庄家</p><div className="extra-card-row">{(s?.dealer_revealed ? s.dealer_cards : s?.dealer_cards.slice(0, 1))?.map((card, i) => <PlayingCard code={card} key={i} />)}{s && !s.dealer_revealed && <span className="extra-card-back" role="img" aria-label="庄家暗牌，尚未公开"><span aria-hidden="true">◇</span></span>}{!s && <span className="extra-table-empty">等待 Deal 后显示已确认手牌</span>}</div>{s?.dealer_revealed && s.dealer_total ? <p aria-label="庄家总点数">{s.dealer_total.is_soft ? '软' : '硬'} {s.dealer_total.best_total} 点</p> : s ? <p>暗牌保持隐藏 · 预检查不透露牌面</p> : <p>6 副牌 / S17 / Natural 3:2</p>}</div>
                <div className="extra-table-arc" aria-hidden="true"><span>BLACKJACK PAYS 3:2</span></div>
                ...
                <div className="extra-hand-lanes">{hands.map((hand, i) => <article key={hand.hand_id} data-testid="blackjack-hand" data-hand-id={hand.hand_id} className={`extra-hand${hand.hand_id === active?.hand_id ? ' is-active' : ''}${hand.hand_id === inspected ? ' is-inspected' : ''}`} aria-label={`第 ${i + 1} 手${hand.hand_id === active?.hand_id ? '，当前行动' : ''}`}>
                    ...
                    <div className="extra-card-row">{hand.cards.map((card, index) => <PlayingCard code={card} key={index} />)}</div>
            ...
            <aside className="extra-console">
                {!s || s.phase === 'SETTLED' ? <ExtraWager controls={props} blocked={blocked} label="初始下注 · 筹码" verb="Deal" onSubmit={units => void command.run(() => props.onDeal(units))} /> : <div className="extra-action-dock">
                    ...
                    <div className="extra-main-actions"><button type="button" disabled={blocked || !active || !legal.includes('HIT')} onClick={() => act('HIT')}>Hit · 要牌</button><button type="button" disabled={blocked || !active || !legal.includes('STAND')} onClick={() => act('STAND')}>Stand · 停牌</button></div>
                    <div className="extra-add-actions">{(['DOUBLE', 'SPLIT'] as const).filter(type => legal.includes(type)).map(type => <button key={type} type="button" disabled={blocked || !canAdd} onClick={() => act(type)}>{type === 'DOUBLE' ? 'Double' : 'Split'} · +{formatChipUnits(active?.stake_units || null)} 筹码</button>)}</div>
                    ...
                    <p className="extra-note">仅展示服务端允许的追加动作，不提供策略推荐。</p>
                </div>}
