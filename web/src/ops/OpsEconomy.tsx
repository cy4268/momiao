...
import { OpsGamblerCampaign } from './OpsGamblerCampaign';
...
export function OpsEconomy(){
 ...
 return <><header className="page-heading"><div><p className="eyebrow">OPERATIONS / ECONOMY</p><h1>经济与账务</h1><p>核对本地钱包规模、额度转账和单个账户的统一资产。</p></div><button onClick={overview.reload} disabled={overview.loading}>刷新总览</button></header>{overview.loading?<Loading/>:overview.error?<Alert>{overview.error}</Alert>:overview.data&&<section className="panel ops-business-economy-overview"><div className="section-heading"><h2>账务总览</h2></div><p className="hint">观察时间：{opsTime(overview.data.observed_at)}</p><dl className="ops-impact"><div><dt>钱包账户数</dt><dd>{overview.data.wallet_accounts}</dd></div><div><dt>Reserve API Credit</dt><dd>{opsAmount(overview.data.reserve_units)}</dd></div><div><dt>可用筹码</dt><dd>{opsAmount(overview.data.available_chips_units)}</dd></div><div><dt>交易记录数</dt><dd>{overview.data.transactions}</dd></div><div><dt>待完成转账</dt><dd>{overview.data.pending_transfers} 笔 · {opsAmount(overview.data.pending_transfer_units)}</dd></div><div><dt>需核对转账</dt><dd>{overview.data.needs_review_transfers} 笔 · {opsAmount(overview.data.needs_review_transfer_units)}</dd></div></dl></section>}<section className="panel ops-business-account"><h2>账户资产查询</h2><form className="filters ops-business-account-filter" onSubmit={filter}><label>账户编号<input value={input} onChange={event=>setInput(event.target.value)} maxLength={19} inputMode="numeric" required/></label><button type="submit" disabled={account.loading&&!!user}>查询账户</button></form>{error&&<Alert>{error}</Alert>}{user&&(account.loading?<Loading/>:account.error?<Alert>{account.error}</Alert>:account.data&&<><div className="ops-business-account-grid"><div className="ops-business-asset-summary"><h3>账户 {account.data.assets.user_id}</h3><dl className="ops-impact">{assetFields.map(([key,label])=><div key={key}><dt>{label}</dt><dd>{opsAmount(account.data!.assets[key])}</dd></div>)}<div><dt>统一资产合计</dt><dd>{opsAmount(account.data.assets.total_units)}</dd></div></dl><p className="hint">本地观察：{opsTime(account.data.assets.observed_at)}；Native 观察：{opsTime(account.data.assets.native_observed_at)}。</p><details><summary>账户迁移与注册奖励</summary><OpsFacts value={account.data.migration}/></details></div><OpsAdjustment key={user} user={user} wallets={account.data.wallets}/></div></>)}</section>{user&&<OpsLedger key={user} user={user}/>}<OpsTransfers/><OpsGamblerCampaign/></>;
}
