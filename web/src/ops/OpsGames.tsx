...
const rouletteStatsSchema=z.object({escrow_units:aggregate,funded_units:aggregate,refunded_units:aggregate,paid_units:aggregate,conservation_delta:aggregate,completed:decimal,surrendered:decimal,draws:decimal,timeouts:decimal,needs_review:decimal,review_rooms:z.array(z.object({id:uuid,state:z.string(),version:decimal,escrow_units:aggregate,anomaly_code:z.string(),created_at:time})).max(100)});
const detailSchema=z.object({generated_at:time,game:gameSchema,config_versions:z.array(configSchema).max(100),validation_artifacts:z.array(artifactSchema).max(100),baseline_locked:z.literal(true),roulette:rouletteStatsSchema.optional()});
...
export function OpsProbabilityDetails({gameSlug,config,artifacts}:{gameSlug:string;config:OpsConfig;artifacts:OpsArtifact[]}){
 ...
 else if(artifact.artifact_type==='MULTIPLAYER_RULESET')content=<p>全体玩家零抽水，个人胜率取决于玩法。固定规则版本不使用单人概率表。</p>;
...
export function OpsGames(){
 ...
 const isRoulette=slug==='devil-roulette'||slug==='pressure-roulette';
 ...
 {config&&<><OpsFacts value={config}/><OpsProbabilityDetails gameSlug={game.game_slug} config={config} artifacts={detail.data!.validation_artifacts}/><form className="ops-business-lifecycle" onSubmit={configAction}><fieldset disabled={blocked}><legend>配置生命周期</legend><label>操作<select name="operation_type">{isRoulette?<><option value="ROULETTE_CONFIG_PREVIEW" disabled={config.status!=='VALIDATED'||!can('ROULETTE_CONFIG_PREVIEW')}>预览固定规则版本</option><option value="ROULETTE_CONFIG_ACTIVATE" disabled={config.status!=='PREVIEWED'||!can('ROULETTE_CONFIG_ACTIVATE')}>激活已预览版本</option></>:<><option value="GAME_CONFIG_CLONE_DRAFT" disabled={!can('GAME_CONFIG_CLONE_DRAFT')}>从此版本建立草稿</option><option value="GAME_CONFIG_VALIDATE" disabled={config.status!=='DRAFT'||!can('GAME_CONFIG_VALIDATE')}>验证草稿</option><option value="GAME_CONFIG_PREVIEW" disabled={config.status!=='VALIDATED'||!can('GAME_CONFIG_PREVIEW')}>生成配置预览</option><option value="GAME_CONFIG_ACTIVATE" disabled={config.status!=='PREVIEWED'||!can('GAME_CONFIG_ACTIVATE')}>激活已预览配置</option></>}</select></label><label>操作原因<textarea name="reason" maxLength={2048} required/></label><button type="submit">读取配置操作影响</button></fieldset></form></>}
 {config?.editable_config&&config.status==='DRAFT'&&<form className="ops-business-draft" onSubmit={saveDraft} key={config.config_version_id+config.version_number}><fieldset disabled={blocked||!can('GAME_CONFIG_DRAFT_SAVE')}><legend>编辑奖池草稿</legend><label>奖池版本<input name="prize_table_version" defaultValue={config.editable_config.prize_table_version} maxLength={128} required/></label>{config.editable_config.prizes.map((prize,index)=><label key={prize.tier}>{prize.tier} · 固定倍率 {prize.multiplier}<input name={`weight_${index}`} defaultValue={prize.weight} inputMode="numeric" maxLength={19} required/></label>)}<label>操作原因<textarea name="reason" maxLength={2048} required/></label><button type="submit">预览草稿变更</button></fieldset></form>}
 {detail.data!.roulette&&<section className="ops-business-review"><h3>多人资金守恒与待核对</h3><p>单位为最小筹码单位；在局托管不属于钱包可花余额。待核对列表仅展示最早 100 局。</p><OpsFacts value={detail.data!.roulette}/>{detail.data!.roulette.review_rooms.map(room=><form key={room.id} onSubmit={event=>{event.preventDefault();mutation.prepare({operation_type:'ROULETTE_SYSTEM_VOID',target:{type:'roulette_round',id:room.id,expected_version:room.version},input:{},reason:String(new FormData(event.currentTarget).get('reason')||'').trim()})}}><fieldset disabled={blocked||!can('ROULETTE_SYSTEM_VOID')}><legend>核对 {room.id} · {room.anomaly_code}</legend><label>作废原因<textarea name="reason" required maxLength={2048}/></label><button>预览原净托管退款</button></fieldset></form>)}</section>}
