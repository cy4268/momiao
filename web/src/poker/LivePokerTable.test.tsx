...
describe('Live Table policy presentation boundary',()=>{
  it.each(['action','takeover','retry','control-retry'])('blocks %s at the real Table UI without revoking read-only queries',kind=>{
    ...
    if(kind==='retry'){expect(screen.getByRole('button',{name:'核对原回执'})).toBeEnabled();fireEvent.click(screen.getByRole('button',{name:'核对原回执'}));expect(query).toHaveBeenCalledTimes(1);}
  });
});
...
describe('Live Table native lifecycle and real transport binding',()=>{
  ...
  it('soft policies preserve UNKNOWN and saved callbacks use latest gate; query/retry use original IDs',async()=>{
    ...
    const original=actions()[0];r.fetcher.mockImplementation(async(path,init)=>path.endsWith('/connect-tickets')?ok({poker_connect_ticket:'ct1.eyJzeW50aGV0aWMiOnRydWV9.c2lnbmF0dXJl'}):path.endsWith('/receipt-query')?ok({table_id:table,kind:JSON.parse(init!.body as string).kind,mutation_id:original.action_id,state:'NOT_FOUND'}):ok({}));
    ...
    fireEvent.click(screen.getByRole('button',{name:'核对原回执'}));await act(async()=>{});expect(calls(r,'/receipt-query').at(-1)![1]!.body).toContain(original.action_id);
  ...
  it('does not turn MAINTENANCE metadata or a same-native-session token refresh into a hard boundary',async()=>{
    ...
    expect(r.api.getSessionGeneration()).toBe(generation);expect(calls(r,'/connect-tickets')).toHaveLength(1);expect(observed.current?.authority.can_control).toBe(true);fireEvent.click(screen.getByRole('button',{name:/跟注 .* Chips/}));expect(actions()).toHaveLength(1);
  });
  ...
  it('forwards current/historical control queries and original-key Retry while separating READ_ONLY connection and TakeOver',async()=>{
    ...
    v.rerender(<LivePokerTable {...r.p} mutation_blocked/>);fireEvent.click(screen.getByRole('button',{name:'核对接管回执'}));await act(async()=>{});act(()=>retry(context()));expect(calls(r,'/take-over')).toHaveLength(1);expect(screen.getByRole('button',{name:'重试本次接管'})).toBeDisabled();
    ...
    expect(observed.current?.authority.ticket_intent).toBe('READ_ONLY');expect(observed.current?.authority.control_history?.count).toBe(1);fireEvent.click(screen.getByRole('button',{name:'核对上次接管回执'}));await act(async()=>{});expect(calls(r,'/receipt-query').at(-1)![1]!.body).toContain(original);
...
describe('Original Session safe-exit binding, not a receipt or empty seat inference',()=>{
  it('keeps the original Session query visible during reconnect without a Table snapshot',async()=>{
    ...
    expect(screen.getByRole('button',{name:'核对原会话出金'})).toBeEnabled();r.setRead(async()=>ok(settled()));fireEvent.click(screen.getByRole('button',{name:'核对原会话出金'}));await act(async()=>{});expect(r.p.onReturnLobby).toHaveBeenCalledTimes(1);
  });
  ...
  it('singleflights queries, stops after read error, manual retry keeps A, and soft gate retains exit checking',async()=>{
    const r=await setup(),d=deferred<Response>();r.setRead(()=>d.promise);const v=await live(r);vi.useFakeTimers();await leave();
    fireEvent.click(screen.getByRole('button',{name:'核对原会话出金'}));fireEvent.click(screen.getByRole('button',{name:'核对原会话出金'}));expect(calls(r,'/sessions/'+session)).toHaveLength(1);
    await act(async()=>{d.resolve(new Response('',{status:503}));});await act(async()=>{vi.advanceTimersByTime(6000);});expect(calls(r,'/sessions/'+session)).toHaveLength(1);
    v.rerender(<LivePokerTable {...r.p} mutation_blocked/>);r.setRead(async()=>ok(settled()));fireEvent.click(screen.getByRole('button',{name:'核对原会话出金'}));await act(async()=>{});expect(r.p.onReturnLobby).toHaveBeenCalledTimes(1);expect(calls(r,'/safe-leave')).toHaveLength(1);
  });
