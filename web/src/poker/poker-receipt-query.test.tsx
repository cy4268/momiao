...
describe('pending receipt query presentation',()=>{
  it('keeps readonly recovery automatic and all new mutations disabled while status details stay folded',async()=>{
    const r=await setup(),s=r.client.getSnapshot()!,query=vi.fn(),mutation=vi.fn();const props={table:s.table!,authority:{...pokerStreamAuthority(s),pending:true},ui:{bet:{action_type:'RAISE' as const,amount_chips:'1'},top_up_chips:'1',confirm_leave:false,confirm_takeover:false},onUiChange:vi.fn(),onIntent:mutation,onQueryReceipt:query,receipt_querying:false};const view=render(<PokerTable {...props}/>);expect(screen.getByRole('status')).toHaveTextContent('正在自动确认操作结果');expect(screen.queryByRole('dialog',{name:'连接与操作状态'})).not.toBeInTheDocument();expect(screen.queryByRole('button',{name:'核对原回执'})).not.toBeInTheDocument();for(const b of screen.getAllByRole('button'))if(b.textContent?.includes('跟注')){expect(b).toBeDisabled();fireEvent.click(b);}expect(mutation).not.toHaveBeenCalled();fireEvent.click(screen.getByRole('button',{name:'查看状态'}));expect(screen.getByRole('dialog',{name:'连接与操作状态'})).toHaveTextContent('系统会自动读取原请求的结果与最新牌局');expect(query).not.toHaveBeenCalled();view.rerender(<PokerTable {...props} receipt_querying/>);expect(screen.queryByRole('button',{name:'核对原回执'})).not.toBeInTheDocument();expect(query).not.toHaveBeenCalled();
  });
});
