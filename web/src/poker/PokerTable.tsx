import { CapReceipt, CapRules } from '../economy-cap';
...
export function PokerTable(p: PokerTableProps) {
  ...
  return <section className="pk-ui pk-table pk-salon" aria-label="沉浸式 Poker 牌桌">
    ...
    <footer className="pk-utility-bar" aria-label="牌桌工具">
      <CapReceipt receipt={t.hand?.economy_settlement}/><div><button onClick={()=>setPanel('rules')}>游戏规则</button><button onClick={()=>setPanel('fairness')}>公平验证</button><button onClick={()=>setPanel('history')}>牌局详情</button><button className="pk-mobile-activity" onClick={()=>setPanel('activity')}>记录与聊天</button>{t.host&&<button onClick={()=>setPanel('host')}>房主管理</button>}</div>
    ...
    {panel&&!p.ui.confirm_leave&&!p.ui.confirm_takeover&&<Modal title={modalTitle[panel]} onClose={()=>setPanel(null)}>
      {panel==='rules'&&<div className="pk-modal-copy"><CapRules active={!!t.hand?.economy_settlement}/><p>无限注德州扑克 · 无 Ante · 新入桌等待大盲。</p><p>首手庄位公平随机，后续依规则轮转。标准高牌型比较，A2345 为五高，花色不破同分。</p><p>下注与加注输入本轮总额，不是额外追加数；快捷金额由服务端按跟注后底池、整 Chip 向下取整提供。只显示当前合法行动。</p><p>暂离与安全离座由服务端执行；正在参与或已弃牌的本手先完成结算，随后才完成出金。</p></div>}
