...
export function PokerHostControls(p:PokerHostControlsProps) {
  ...
  return <section className="pk-live-panel pk-host" aria-labelledby="pk-host-title">
    ...
    {p.operation&&<div className="pk-host-operation" role="status"><p><strong>{p.operation.phase==='ACKNOWLEDGED'?'命令已受理':'命令结果尚未确认'}</strong>{p.operation.status&&<span> · {p.operation.status}</span>}</p><div>
      {p.operation.phase!=='ACKNOWLEDGED'&&<button disabled={p.operation.querying} aria-busy={p.operation.querying} onClick={()=>{if(!p.operation?.querying)p.onQueryReceipt?.();}}>核对原回执</button>}
    ...
    {confirmation&&<div className="pk-host-confirm" role="alertdialog" aria-modal="true" aria-labelledby="pk-host-confirm-title"><div><strong id="pk-host-confirm-title">{confirmation.label}？</strong><p>{confirmation.detail}</p></div><button className="pk-primary" disabled={!enabled} onClick={submit}>确认提交</button><button onClick={()=>setConfirmation(undefined)}>取消</button></div>}
  </section>;
}
