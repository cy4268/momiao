...
describe('R1 explicit retry presentation keeps ordinary mutation lock',()=>{
  it('sends only rendered context through the separate retry callback and disables it until runtime eligibility',async()=>{
    const r=await unknown();await reconnect(r);const s=r.client.getSnapshot()!,retry=vi.fn(),mutation=vi.fn(),props={table:s.table!,authority:{...pokerStreamAuthority(s),can_retry_pending:true},ui:{bet:{action_type:'RAISE' as const,amount_chips:'999'},top_up_chips:'999',confirm_leave:false,confirm_takeover:false},onUiChange:vi.fn(),onIntent:mutation,onRetryPending:retry};const view=render(<PokerTable {...props}/>);fireEvent.click(screen.getByRole('button',{name:'重试原操作'}));expect(retry).toHaveBeenCalledWith(pokerStreamContext(s));expect(mutation).not.toHaveBeenCalled();view.rerender(<PokerTable {...props} authority={{...props.authority,can_retry_pending:false}}/>);expect(screen.getByRole('button',{name:'重试原操作'})).toBeDisabled();fireEvent.click(screen.getByRole('button',{name:'重试原操作'}));expect(retry).toHaveBeenCalledTimes(1);for(const b of screen.getAllByRole('button'))if(b.textContent?.includes('跟注'))expect(b).toBeDisabled();
  });
});
