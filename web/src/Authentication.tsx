...
export function Authentication({client,mode}: {client:ApiClient;mode:'login'|'registration'}) {
    ...
    const capabilities=client.getSessionCapabilities();
    const discordUnavailable=opaque&&client.sessionFeatureUnavailable('DISCORD_S2');
    const passwordUnavailable=opaque&&capabilities.password?.state!=='AVAILABLE';
    const turnstileRequired=opaque&&capabilities.password?.turnstileRequired===true;
    const [turnstileToken,setTurnstileToken]=useState(''),[turnstileReset,setTurnstileReset]=useState(0);
    const passwordReady=!opaque||(capabilities.authenticationState==='ANONYMOUS'&&!passwordUnavailable&&(!turnstileRequired||!!turnstileToken));
    ...
    const cleanupRequired=opaque&&!challenge&&!['ANONYMOUS','UNINITIALIZED'].includes(capabilities.authenticationState);
    const lock=useRef(false); const active=useRef(true);
    useEffect(()=>{active.current=true;document.title=(registration?'注册':'登录')+' · momiao';if(!discordUnavailable)void client.admissionConfig().then(d=>{if(active.current)setConfig(d);}).catch(()=>{if(active.current)setConfigError('Discord 入口暂时无法读取，请稍后重试。');});return()=>{active.current=false;};},[client,registration,discordUnavailable]);
    ...
    async function passwordLogin(e:FormEvent){e.preventDefault();if(lock.current||session.loggingOut)return;lock.current=true;setBusy(true);setError('');try{const next=await client.login(username.trim(),password,turnstileRequired?turnstileToken:'');if(active.current&&next)setChallenge(next);}catch(e){if(active.current)setError(errorText(e));}finally{setPassword('');setShow(false);setTurnstileToken('');setTurnstileReset(value=>value+1);if(active.current){lock.current=false;setBusy(false);}}}
    return <AuthFrame registration={registration}><p className="eyebrow">{registration?'A NEW CONNECTION':'WELCOME ABOARD'}</p><h2>{challenge?'验证你的身份':registration?'建立你的账户':'欢迎回来'}</h2><p className="subtitle">{challenge?'输入验证器验证码或备用码，完成本次登录。':registration?'使用 Discord 验证注册资格，随后建立 Master 资料。':'使用已绑定的 Discord 或原生账户密码登录。'}</p>
        {session.notice&&<Alert>{session.notice}{opaque&&<button disabled={session.loggingOut||busy} onClick={()=>void client.refresh().catch(e=>{if(active.current)setError(errorText(e));})}>重新核对会话</button>}{(cleanupRequired||session.notice.includes('服务端退出未确认'))&&<button disabled={session.loggingOut} onClick={()=>void client.logout().catch(e=>{if(active.current)setError(errorText(e));})}>{session.loggingOut?'正在清理…':session.notice.includes('服务端退出未确认')?'重试退出':'清理本次会话'}</button>}</Alert>}
        {challenge?<TwoFactorEntry busy={busy} onSubmit={async code=>{const next=await client.verify2fa(challenge.flow_token,code);setChallenge(next||undefined);}} onCancel={()=>{setChallenge(undefined);setError(opaque?'本次验证已取消，正在清理会话；刷新页面后可重新开始。':'');if(opaque)void client.logout().catch(e=>{if(active.current)setError(errorText(e));});}}/>:<>
            {discordUnavailable&&<Alert>Discord 登录与注册尚未接入不透明会话模式（DISCORD_S2）。</Alert>}
            {!discordUnavailable&&config?.enabled&&<div className="auth-discord"><button className="primary" disabled={busy||session.loggingOut||(registration&&!config.registration_enabled)} onClick={()=>void discord()}>{busy?'正在连接…':registration?'使用 Discord 注册':'使用 Discord 登录'}<span aria-hidden="true">↗</span></button>{registration&&<p className="hint">{config.eligibility}</p>}</div>}
            {!discordUnavailable&&!config&&!configError&&<Loading/>}{!discordUnavailable&&configError&&<p className="hint" role="status">{configError}</p>}{!discordUnavailable&&registration&&config&&!config.registration_enabled&&<p role="status">新用户注册暂未开放，请稍后再来。</p>}
            {!registration&&<><div className="auth-divider"><span>{opaque?'使用密码':'或使用密码'}</span></div>{turnstileRequired&&capabilities.password?.turnstileSiteKey&&<HumanCheck siteKey={capabilities.password.turnstileSiteKey} reset={turnstileReset} onToken={setTurnstileToken}/> }{passwordUnavailable&&<Alert>后端当前未提供可用的密码登录能力。</Alert>}{opaque&&!['ANONYMOUS','TWO_FA'].includes(capabilities.authenticationState)&&<Alert>当前会话状态为 {capabilities.authenticationState}，请先按提示核对或清理，避免重复提交。</Alert>}<form onSubmit={e=>void passwordLogin(e)}><label htmlFor="login-username">用户名</label><input id="login-username" autoComplete="username" value={username} onChange={e=>setUsername(e.target.value)} required maxLength={128} placeholder={opaque?'登录标识':'原生登录标识'} disabled={busy||!passwordReady}/><PasswordInput id="login-password" label="密码" value={password} setValue={setPassword} show={show} setShow={setShow} autoComplete="current-password" disabled={busy||!passwordReady}/><button className="primary auth-submit" disabled={busy||session.loggingOut||!passwordReady} type="submit">{session.loggingOut?'正在退出…':busy?'正在验证…':'登录控制台'}<span aria-hidden="true">↗</span></button></form></>}
...
export function DiscordCallback({client,captured,onComplete}: {client:ApiClient;captured:CapturedCallback;onComplete:(proof?:SensitiveProof)=>void}) {
    ...
    function complete(result:AdmissionResult){if(!active.current)return;if(result&&'require_2fa'in result)setChallenge(result);else onComplete(result&&('proof'in result||'purpose'in result)?result:undefined);}
    useEffect(()=>{active.current=true;document.title='身份验证 · momiao';if(!started.current){started.current=true;if(client.sessionFeatureUnavailable('DISCORD_S2'))setError('Discord 授权回调尚未接入不透明会话模式（DISCORD_S2）。');else if(captured.input){const input=captured.input;captured.input=undefined;void client.discordCallback(input).then(complete).catch(e=>{if(active.current)setError(admissionError(e));});}else if(!captured.error)setError('授权回调已失效，请重新开始。');}return()=>{active.current=false;};},[client,captured]);
