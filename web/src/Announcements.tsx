...
import art from './announcement-art.json';
import './announcements.css';
import './announcement-chamber.css';

function AnnouncementShell({ client, children }: { client: ApiClient; children: ReactNode }) {
    const signedIn = !!client.getSnapshot().user;
    return <div className='announcement-site'><img className='announcement-backdrop' src={assetUrl(art.background)} alt='' decoding='async' /><a className='skip-link' href='#announcement-content'>跳至公告内容</a><header className='public-header'><Link to='/' className='brand' aria-label='Chaldea Platform 首页'><Brand /></Link><nav aria-label='公告导航'><Link to='/models'>模型目录</Link><Link to='/entertainment'>娱乐</Link><Link to='/rankings'>排行榜</Link><Link to='/announcements' aria-current='page'>公告</Link><Link to={signedIn ? '/dashboard' : '/login'}>{signedIn ? '指挥台' : '登录账户'}</Link></nav></header><main id='announcement-content' className='announcement-main' tabIndex={-1}><div className='announcement-panel'>{children}</div></main><footer className='public-footer'><span>Chaldea Platform / 公告与更新</span><nav><Link to='/'>返回首页</Link>{signedIn && <Link to='/ops/announcements'>公告运营</Link>}</nav></footer></div>;
}
...
export function Announcements({ client }: { client: ApiClient }) {
    ...
    return <AnnouncementShell client={client}><header className='announcement-title'><p className='eyebrow'>CHALDEA / COMMUNICATIONS</p><h1>公告与更新</h1><p>平台消息、维护安排，以及每一份值得记住的支持。</p></header><div className='announcement-tabs' aria-label='公告范围'><button aria-pressed={!archive} onClick={() => { const p = new URLSearchParams(params); p.delete('archive'); p.delete('offset'); setParams(p); }}>当前公告</button><button aria-pressed={archive} onClick={() => { const p = new URLSearchParams(params); p.set('archive', 'true'); p.delete('offset'); setParams(p); }}>历史归档</button></div><form className='announcement-filters' onSubmit={apply}><label>公告类别<select value={type} onChange={e => setType(e.target.value)}><option value=''>全部类别</option>{Object.entries(announcementTypes).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label className='announcement-search'>搜索公告<input value={search} onChange={e => setSearch(e.target.value)} maxLength={200} placeholder='标题或正文关键词' /></label><label>开始日期<input type='date' value={from} onChange={e => setFrom(e.target.value)} /></label><label>结束日期<input type='date' value={to} onChange={e => setTo(e.target.value)} /></label><button type='submit'>应用筛选</button></form><p className='hint'>日期与时间按上海时区显示。{archive ? '这里保留曾经发布的历史公告。' : '置顶公告按人工顺序排列，其余按发布时间排列。'}</p>
        {resource.loading ? <p role='status'>正在读取公告…</p> : resource.error ? <><Alert>{resource.error}</Alert><button onClick={resource.reload}>重新读取</button></> : resource.data?.items.length ? <><div className='announcement-list' role='region' aria-label='公告列表' tabIndex={0}>{resource.data.items.map(item => <article className='announcement-row' key={item.announcement_id}><div className='announcement-row-meta'><span data-type={item.type}>{announcementTypes[item.type]}</span>{item.pinned && !archive && <b>置顶</b>}</div><h2><Link to={'/announcements/' + item.announcement_id + (archive ? '?archive=true' : '')}>{item.title}</Link></h2><time dateTime={item.publish_at || undefined}>{announcementTime(item.publish_at)}</time>{client.getSnapshot().user && <small className='announcement-read' data-read={item.read}>{item.read ? '已读' : '未读'}</small>}<span className='announcement-row-arrow' aria-hidden='true'>↗</span></article>)}</div><nav className='announcement-pager' aria-label='公告分页'><button disabled={offset <= 0} onClick={() => page(Math.max(0, offset - 20))}>上一页</button><span>第 {Math.floor(offset / 20) + 1} 页</span><button disabled={!resource.data.has_more || offset >= 10000} onClick={() => page(offset + 20)}>下一页</button></nav></> : <Empty title='没有符合条件的公告'>可以调整筛选条件，或稍后再来查看。</Empty>}
    </AnnouncementShell>;
}
export function AnnouncementDetail({ client }: { client: ApiClient }) {
    ...
    return <AnnouncementShell client={client}><Link className='announcement-back' to={'/announcements' + (archive ? '?archive=true' : '')}>← {archive ? '历史归档' : '全部公告'}</Link>{resource.loading ? <p role='status'>正在读取公告…</p> : resource.error ? <><Alert>{resource.error}</Alert><div className='announcement-actions'><button onClick={resource.reload}>重新读取</button>{!client.getSnapshot().user && <Link to='/login'>登录账户</Link>}<Link to='/announcements?archive=true'>查看历史归档</Link></div></> : resource.data && <article className='announcement-detail'><header><p className='eyebrow'>{announcementTypes[resource.data.type]} / {announcementStates[resource.data.state]}</p><h1>{resource.data.title}</h1><dl className='announcement-dates'><div><dt>发布时间</dt><dd>{announcementTime(resource.data.publish_at)}</dd></div><div><dt>最近更新</dt><dd>{announcementTime(resource.data.updated_at)}</dd></div>{resource.data.visible_until && <div><dt>展示截止</dt><dd>{announcementTime(resource.data.visible_until)}</dd></div>}</dl></header><div className='announcement-reading' role='region' aria-label='公告正文' tabIndex={0}><AnnouncementBody item={resource.data} />{readError && <Alert>正文已读取，阅读状态暂未保存。{readError}<button onClick={() => setReadAttempt(v => v + 1)}>重试保存阅读状态</button></Alert>}</div></article>}</AnnouncementShell>;
}
