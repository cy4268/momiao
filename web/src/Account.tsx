...
export function Account({client,proof,clearProof=()=>{}}: {client:ApiClient;proof?:SensitiveProof;clearProof?:()=>void}) {
    ...
    if(opaqueBlocked)return <div><header className="page-heading"><div><p className="eyebrow">MY / ACCOUNT &amp; SECURITY</p><h1>账户与安全</h1><p>当前仅显示已核对的不透明会话身份。</p></div><button disabled={loading||session.loggingOut} onClick={()=>void reload()}>重新核对会话</button></header>{error&&<Alert>{error}</Alert>}<Alert>密码变更尚未接入不透明会话模式；账户详情与 Discord 连接同属 GENERAL_BFF_S2，当前不会调用旧原生端点。</Alert><section className="panel"><p className="eyebrow">VERIFIED SESSION IDENTITY</p><h2>已核对身份</h2><dl className="account-details"><div><dt>登录标识 · 只读</dt><dd>{session.user?.username||'会话待核对'}</dd></div><div><dt>平台用户 ID · 只读</dt><dd>{session.user?.id||'—'}</dd></div></dl><p className="hint">密码、Discord 连接状态和账户扩展资料将在对应 BFF 契约接入后开放。</p><button disabled={session.loggingOut} onClick={()=>{clearProof();void client.logout().catch(()=>{});}}>{session.loggingOut?'正在退出…':'退出当前登录'}</button></section></div>;
