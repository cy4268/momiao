import { capSettlementSchema, economicPolicySchema } from '../economy-cap';
import {z} from 'zod';
...
const lobbySchema=z.object({economic_policy:economicPolicySchema.optional(),game:z.enum(rouletteSlugs),state:z.enum(['PLAY','MAINTENANCE','COMING_SOON','RETIRED','TEMPORARILY_UNAVAILABLE']),binding:bindingSchema,minimum_units:int64,step_units:int64,available_units:int64,own_round_id:uuid.nullable(),rooms:z.array(lobbyRoom).max(50),next_cursor:z.string().max(256).nullable(),csrf_token:z.string().regex(/^[0-9a-f]{64}$/)}).strict();
