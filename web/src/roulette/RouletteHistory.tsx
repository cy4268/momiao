...
import {HistoryArchive} from '../history/HistoryArchive';
...
function Content({client,id}:{client:ApiClient;id:string}){
 ...
 return <HistoryArchive title={`${d?rouletteNames[d.game]:'轮盘'} · 对局记录`} detail={'Round · '+id} navigation={<Back/>}>
  ...
  {read.loading?<Loading/>:read.error?<Alert>{read.error}<button onClick={()=>setCursor('')}>返回第一页</button></Alert>:d&&<>
   ...
   <section className="panel archive-detail-panel"><h2>逐步回放</h2>{d.frames.length>0?<><label>本页动作<select value={frame} onChange={e=>setFrame(Number(e.target.value))}>{d.frames.map((f,i)=><option value={i} key={f.sequence}>#{f.sequence} · {d.actions[i]?.text}</option>)}</select></label>{d.game==='pressure-roulette'?<PressureTable view={d.frames[Math.min(frame,d.frames.length-1)].view} onAction={()=>{}} busy timer="历史回放"/>:<DevilTable view={d.frames[Math.min(frame,d.frames.length-1)].view} onAction={()=>{}} busy timer="历史回放"/>}</>:<p>结束前只展示公共事件与自己的视图。</p>}
    <details className="history-disclosure"><summary>本页行动时间线</summary><ol>{d.actions.map(a=><li key={a.sequence}>#{a.sequence} · {a.text}</li>)}</ol></details>
    <div className="ops-actions"><button disabled={!cursor} onClick={()=>{setCursor('');setFrame(0)}}>第一页</button><button disabled={!d.next_cursor} onClick={()=>{setCursor(d.next_cursor!);setFrame(0)}}>下一页动作</button></div>
   </section>
   <details className="panel history-disclosure"><summary>钱包交易回链</summary><ul>{d.transaction_ids.map(v=><li key={v}><Link to={'/wallet/transactions/'+v}>{v}</Link></li>)}</ul>{d.transactions_truncated&&<p>此处展示前 100 笔；完整资金核对由证明逐笔验证。</p>}</details>
  </>}</div>
 </HistoryArchive>;
}
