...
export function DevilTable({view,onAction,busy,timer}:{view:RoomView;onAction:(action:Action)=>void;busy:boolean;timer:string}){
 ...
 return <section className="roulette-table roulette-devil" aria-label="恶魔轮盘桌面">
  ...
  <div className="roulette-gun-stage"><div className="roulette-orbit" aria-hidden="true"/><img src="/roulette/devil-shotgun.png" className="roulette-gun" alt=""/></div>
  ...
  <div className="roulette-action-bar"><div className="roulette-items">{unique.length?unique.map(item=>{
   ...
   return <div className="roulette-item" key={item}><RouletteArt name={item} className="roulette-item-icon"/><strong>{itemNames[item]} <small>×{ownItems.filter(v=>v===item).length}</small></strong><small>{descriptions[item]}</small>
    {item==='adrenaline'?<div>{actions.map(a=><button key={a.kind==='DEVIL_ITEM'?a.stolen_item:''} disabled={busy} onClick={()=>onAction(a)}>偷取 {a.kind==='DEVIL_ITEM'&&itemNames[a.stolen_item!]}</button>)}{!actions.length&&<button disabled>等待可用目标</button>}</div>:<button disabled={busy||!actions.length} onClick={()=>actions[0]&&onAction(actions[0])}>使用</button>}
   </div>;
