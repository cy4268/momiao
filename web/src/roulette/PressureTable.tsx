...
import {RouletteArt,rouletteImage,seatCrests} from './RouletteArt';
...
export function PressureTable({view,onAction,busy,timer}:{view:RoomView;onAction:(action:Action)=>void;busy:boolean;timer:string}){
 ...
 return <section className="roulette-table roulette-pressure" aria-label="加压轮盘桌面" data-state={view.state} data-phase={p?.phase??view.state} style={{'--roulette-table-art':`url("${rouletteImage('table')}")`} as CSSProperties}>
  ...
  <div className="roulette-roundtable"><div className="roulette-pressure-seats">{all.map((seat,i)=>{
   ...
   return <div key={seat} style={{'--seat-x':`${50+40*Math.cos(angle*Math.PI/180)}%`,'--seat-y':`${50+37*Math.sin(angle*Math.PI/180)}%`} as CSSProperties} className={'roulette-player '+(view.turn_seat===seat?'is-turn':'')+(!player?.alive&&player?' is-out':'')}>
  ...
  <div className="roulette-pressure-center"><img src={rouletteImage('revolver')} alt=""/>
  ...
  <p className="roulette-pressure-prompt">{view.state==='WAITING'?'全员准备，危局开启':p?.phase==='VOTE'?'收场，还是再来一轮？':p?.phase==='CHOICE'?'扳机之后，选择在你':p?.phase==='ENDED'?'圆桌的命运已定':'下一枪，谁来承担？'}</p>
