import { CapReceipt, CapRules, capResult } from './economy-cap';
...
export function GamePage({client,userID,slug}:{client:ApiClient;userID:string;slug:GameSlug}) {
    ...
    const invalid=slug==='dice'&&!choice?'请先选择大或小。':cost===null?'最低下注 10 筹码，只接受整数筹码。':bootstrap?.economic_policy&&cost>BigInt(bootstrap.economic_policy.single_player_max_units)?'整局累计下注上限 1,000,000 筹码。':available!==null&&cost>available?'可用筹码不足。':'';
...
export function RoundReceipt({round,compact=false}:{round:GameRound;compact?:boolean}) {
    ...
    const d=round.dice,actualResult=capResult(round.economy_settlement,round.common_result||'LOSS') as Outcome;
    return <section className={'round-receipt result-'+actualResult+(compact?' compact-receipt':'')} aria-label="本局结果"><div>{!compact&&<p className="eyebrow">SETTLED / 本局已结算</p>}<h2>{outcomeNames[actualResult]} <strong>{chips(round.economy_settlement?.actual_net_units??round.net_change_units,true)}</strong></h2>
        ...
        <dl><div><dt>总消耗</dt><dd>{chips(round.total_stake_units)}</dd></div><div><dt>到账派彩</dt><dd>{chips(round.economy_settlement?.credited_payout_units??round.total_payout_units)}</dd></div><div><dt>结算后余额</dt><dd>{chips(round.balance_after_units)}</dd></div></dl>
        <CapReceipt receipt={round.economy_settlement}/>
    </section>;
}
...
export function GameHistory({client}:{client:ApiClient}) {
    ...
    {history.loading?<Loading/>:history.error?<Alert>{history.error}</Alert>:history.data?.items.length?<div className="table-wrap"><table aria-label="我的游戏记录"><thead><tr><th>游戏 / 时间</th><th>总消耗</th><th>总派彩</th><th>净变化</th><th>状态 / 详情</th></tr></thead><tbody>{history.data.items.map(r=><tr key={r.id}><td>{gameNames[r.game]}<small>{new Date(r.created_at).toLocaleString('zh-CN')}</small></td><td>{chips(r.total_stake_units)}</td><td>{r.state==='SETTLED'?chips(r.economy_settlement?.credited_payout_units??r.total_payout_units):'待结算'}</td><td>{r.state==='SETTLED'?chips(r.economy_settlement?.actual_net_units??r.net_change_units,true):'—'}<small>{r.common_result?outcomeNames[capResult(r.economy_settlement,r.common_result)]:'进行中'}</small></td><td><Link to={'/history/rounds/'+r.id}>{r.state==='SETTLED'?'已结算':'进行中'} · 查看本局 →</Link></td></tr>)}</tbody></table></div>:<Empty title="还没有游戏记录">手动开始一局后，已结算结果会保存在这里。</Empty>}
...
export function GameRoundDetail({client}:{client:ApiClient}) {
    ...
    return <div className="game-round-detail"><header className="page-heading"><div><p className="eyebrow">ROUND / DETAILS</p><h1>本局详情</h1><p className="round-id">{id}</p></div><Link to="/history" className="button">← 返回游戏记录</Link></header>{r.loading?<Loading/>:r.error?<Alert>{r.error}</Alert>:r.data&&<><RoundReceipt round={r.data}/><section className="panel round-authority"><h2>结果明细</h2>{r.data.scratch&&<ol className="history-cells">{r.data.scratch.cells.map((c,i)=><li key={i}>第 {i+1} 格 · ×{c.symbol.slice(1)}{c.matching?' · 匹配星纹':''}</li>)}</ol>}{r.data.summon&&<ol className="history-cells">{r.data.summon.draws.map(d=><li key={d.index}>第 {d.index} 抽 · {d.tier} · ×{d.multiplier}</li>)}</ol>}
        {r.data.slot&&<SlotGame economySettlement={r.data.economy_settlement} result={r.data.slot} rulesetVersion={r.data.ruleset_version} availableUnits={r.data.balance_after_units} wagerChips={r.data.input.type==='SLOT'?r.data.input.total_wager:'10'} onWagerChange={()=>{}} onSpin={async()=>{}} busy={false} disabledReason="历史局仅供回看。返回游戏页后才能创建新局。" roundID={r.data.id}/>}
        {r.data.blackjack&&<BlackjackGame economySettlement={r.data.economy_settlement} snapshot={r.data.blackjack} availableUnits={r.data.balance_after_units} wagerChips={r.data.input.type==='BLACKJACK'?r.data.input.initial_wager:'10'} onWagerChange={()=>{}} onDeal={async()=>{}} onAction={async()=>{}} busy={false} disabledReason="历史快照仅供回看；返回牌桌恢复当前局。" roundID={r.data.id}/>}
