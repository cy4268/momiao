...
import { AnnouncementHomeBanner } from './Announcements';
import { CatalogHomeRecommendations } from './Catalog';

export function Home({ signedIn, client = api }: { signedIn: boolean; client?: ApiClient }) {
    ...
    return <div className="public-home">
        ...
        <header className="public-header"><Link className="brand" to="/" aria-label="Chaldea Platform 首页"><Brand /></Link><nav aria-label="首页导航"><Link className="announcement-nav-link" to="/announcements">公告</Link><a href="#pathways">探索平台</a><Link className="button" to={signedIn ? '/dashboard' : '/login'}>{signedIn ? '打开指挥台' : '登录账户'}</Link></nav></header>
        <main id="home-content" tabIndex={-1}>
            ...
            <AnnouncementHomeBanner client={client} />
            <CatalogHomeRecommendations client={client} />
            <section id="pathways" className="home-pathways" aria-labelledby="pathways-title"><div className="home-section-title"><p className="eyebrow">FIND YOUR WAY</p><h2 id="pathways-title">从这里，走向你的下一站</h2><p>现在可以使用的功能，和明确的下一步。</p></div>
                <article className="home-path api-path"><p className="eyebrow">MODELS & API</p><h3>让灵感，接入模型</h3><p>探索模型能力与公开参考价，选择适合的连接方式。登录后管理独立 API 密钥，在调用记录中核对使用情况。</p><Link className="text-link" to="/models">探索模型目录 <span aria-hidden="true">↗</span></Link><small>模型目录与接入指南公开可读；密钥和个人记录需要登录。</small></article>
        ...
        <footer className="public-footer"><span>momiao / Chaldea Platform</span><nav aria-label="页脚导航"><Link to="/models">模型目录</Link><Link to="/api/access">API 接入</Link>{signedIn && <Link to="/me">个人中心</Link>}<a href="https://github.com/cy4268/momiao" target="_blank" rel="noopener noreferrer">源代码 ↗</a></nav></footer>
    </div>;
}
